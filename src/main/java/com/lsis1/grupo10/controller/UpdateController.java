...
public class UpdateController {
    ...
    public int novaPontuacao(int id, String classificacao, int subtrairPontos) {
        int res = 0;
        if (classificacao == "naoterminou" || classificacao == "saiu") {
            String SQL = "UPDATE associar_robot SET pontos = ? WHERE id = ?;";
            Connection conn = DBConnection.getConnection();
            PreparedStatement ps;
            try {
                ps = conn.prepareStatement(SQL);
        ...
        } else if (classificacao == "0.25" || classificacao == "0.5" || classificacao == "repetiu" || classificacao == null) {
            ...
            try {
                ...
                if (rset.next()) {
                    if (classificacao == "0.25" || classificacao == "repetiu") {
                        pontos = rset.getInt("pontos") + 2;
                        pontos = pontos - subtrairPontos;
                    } else if(classificacao == "0.5"){
